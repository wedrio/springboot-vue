  ...
  <header id="topbar">
    <el-row>
      ...
      <el-col :span="4" class="topbar-left">
        <i class="iconfont icon-kaoshi" style="font-size:16px"></i>
        <span class="title" @click="index()">在线考试系统</span>
      </el-col>

      <el-col :span="20" class="topbar-right">
        <i class="iconfont icon-caidanguanli" @click="toggle()"></i>
        <div class="user">
          <span>{{user.userName}}</span>
          <img src="@/assets/img/icon.png" class="user-img" ref="img" @click="showSetting()" />
...
.topbar-right .icon-caidanguanli {
  font-size: 30px;
  margin-left: 20px;
}
.topbar-right .user {
